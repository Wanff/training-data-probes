from transformers import AutoTokenizer, AutoModelForSequenceClassification, Trainer, TrainingArguments, set_seed, get_linear_schedule_with_warmup, DataCollatorWithPadding
import evaluate
import torch
import os
import numpy as np
...
def preprocess(examples): 
    ...
    if curr_len < max_train_len:
        examples['input_ids'] = [tokenizer.pad_token_id] * (max_train_len - curr_len) + examples['input_ids']
    return examples
...
# print(len(dataset['train']['input_ids']))
...
def compute_metrics(eval_pred):
    ...
    print(f"val_loss: {loss.item()}, accuracy: {acc}, precision: {precision}, recall: {recall}, f1: {f1}, AUC: {auc}, tpr_01: {tpr_01}, tpr_001: {tpr_001}, tpr_0001: {tpr_0001}")
...
# # turn into torch
# dataset['train'] = torch.utils.data.TensorDataset(torch.tensor(dataset['train']['input_ids']), torch.tensor(dataset['train']['labels']))
# dataset['val'] = torch.utils.data.TensorDataset(torch.tensor(dataset['val']['input_ids']), torch.tensor(dataset['val']['labels']))
# train_dataloader = torch.utils.data.DataLoader(dataset['train'], batch_size=batch_size, shuffle=True)
# val_dataloader = torch.utils.data.DataLoader(dataset['val'], batch_size=48, shuffle=False)
...
# # want to use compute_metrics from above
# print("Training...")
...
#         outputs = lora_model(input_ids, labels=batch[1].to('cuda'))
#         loss = outputs.loss
#         loss.backward()
#         optimizer.step()
#         lr_scheduler.step()
...
#     lora_model.save_pretrained(os.path.join(path, 'finetuning/final-llama-on-pythia-2'))
...
targs = TrainingArguments(
    output_dir = os.path.join(path, 'finetuning/llama-on-12b-train-run-2'),
...
model.save_pretrained(os.path.join(path, 'finetuning/llama-2-7b-on-pythia-12b'))