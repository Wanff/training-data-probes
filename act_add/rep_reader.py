#From: https://github.com/andyzoujm/representation-engineering/blob/main/repe/rep_readers.py
...
class RepReader(ABC):
    ...
    def get_signs(self, hidden_states, train_choices, hidden_layers):
        ...
        if self.needs_hiddens and hidden_states is not None and len(hidden_states) > 0 and train_choices is not None:
            for layer in hidden_layers:    
                ...
                for component_index in range(self.n_components):
                    ...
                    self.direction_signs[layer].append(-1 if np.mean(outputs_min) > np.mean(outputs_max) else 1)
        else:
            for layer in hidden_layers:    
                self.direction_signs[layer] = [1 for _ in range(self.n_components)]
...
class PCARepReader(RepReader):
    ...
    def get_rep_directions(self, model, tokenizer, hidden_states, hidden_layers, **kwargs):
        ...
        for layer in hidden_layers:
            ...
            self.directions[layer] = pca_model.components_ # shape (n_components, n_features)
    ...
    def get_signs(self, hidden_states, train_labels, hidden_layers):
        ...
        #just check one of the layers
        assert hidden_states[hidden_layers[0]].shape[0] == len(train_labels), f"Shape mismatch between hidden states ({hidden_states[layer].shape[0]}) and labels ({len(np.concatenate(train_labels))})"
        train_labels = np.array(train_labels).reshape(-1, 2).tolist()
...
class ProbeRepReader(RepReader):    
    ...
    def get_rep_directions(self, hidden_layers, **kwargs):
        ...
        The hidden starts are a dict: {layer: (n_examples hidden_size)}
        where the 2 is for the two classes. 
        """
        self.directions = {}
    ...
    def get_signs(self, hidden_layers):
        self.direction_signs = {}
...
class CAARepReader(RepReader):
    ...
    def get_rep_directions(self, model, tokenizer, hidden_states, hidden_layers, **kwargs):
        ...
        for layer in hidden_layers:
            self.directions[layer] = hidden_states[layer].mean(axis=0)
